<template>
  <div class="page-shell">
    ...
    <p class="text-gray-500 mb-6">认证后可发布校招/实习职位，对接大学生人才。管理员将在1-3个工作日内审核</p>
    ...
    <div v-if="company" class="mb-6 p-4 rounded-xl border"
      ...
      <div class="flex items-center gap-2 mb-2">
        ...
        <span class="font-semibold" :class="company.status === 'approved' ? 'text-green-700' : company.status === 'rejected' ? 'text-red-700' : 'text-yellow-700'">
          {{ { pending: '审核中', approved: '已通过', rejected: '审核未通过' }[company.status] || company.status }}
        </span>
      </div>
      <p class="text-sm text-gray-600">企业名称：{{ company.name }}</p>
      <p class="text-sm text-gray-600">企业类型：{{ company.type === 'individual' ? '个体工商户' : '企业' }}</p>
      <p v-if="company.rejectReason" class="text-sm text-red-600 mt-2">拒绝原因：{{ company.rejectReason }}</p>
      <button v-if="company.status === 'rejected'" @click="editing = true" class="mt-3 px-4 py-2 bg-indigo-600 text-white text-sm rounded-lg hover:bg-indigo-700">重新提交</button>
    </div>
    ...
    <form v-if="!company || editing" @submit.prevent="handleSubmit" class="space-y-6">
      <!-- 类型选择 -->
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-2">认证类型 <span class="text-red-500">*</span></label>
        <div class="grid grid-cols-2 gap-3">
          <button type="button" @click="form.type = 'company'"
            ...
            <div class="font-medium">企业</div>
            <div class="text-xs text-gray-400 mt-1">需营业执照</div>
          </button>
          <button type="button" @click="form.type = 'individual'"
            ...
            class="p-4 border-2 rounded-xl text-center transition-all">
            <div class="text-2xl mb-1">🧑‍💼</div>
            <div class="font-medium">个体工商户</div>
            <div class="text-xs text-gray-400 mt-1">需营业执照+身份证</div>
          </button>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl border border-gray-100 p-5 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-sm text-gray-600 mb-1">企业/店铺名称 <span class="text-red-500">*</span></label>
          ...
          <div>
            <label class="block text-sm text-gray-600 mb-1">法人/负责人姓名 <span class="text-red-500">*</span></label>
          ...
          <div>
            <label class="block text-sm text-gray-600 mb-1">统一社会信用代码 <span class="text-red-500">*</span></label>
            <input v-model="form.creditCode" required class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-sm" placeholder="18位统一信用代码" />
          </div>
          <div>
            <label class="block text-sm text-gray-600 mb-1">所属行业</label>
          ...
          <div>
            <label class="block text-sm text-gray-600 mb-1">所在城市</label>
        ...
        <div>
          <label class="block text-sm text-gray-600 mb-1">企业简介</label>
      ...
      <!-- 认证材料 -->
      <div class="bg-white rounded-xl border border-gray-100 p-5 space-y-4">
        <h3 class="font-semibold text-gray-800">认证材料</h3>
        <p class="text-xs text-gray-400">支持 jpg/png/pdf 格式，每张不超过10MB</p>
      ...
      <div class="flex gap-3">
        <button type="submit" :disabled="submitting"
          ...
          {{ submitting ? '提交中...' : (company ? '重新提交认证' : '提交认证申请') }}
        </button>
        <button v-if="editing" type="button" @click="editing = false"
          class="px-6 py-3 border border-gray-200 rounded-xl text-gray-600 hover:bg-gray-50">取消</button>
      </div>
    </form>
    </template>
  </div>
...
async function handleSubmit() {
  if (!form.name || !form.legalPerson || !form.creditCode) {
    toast('请填写必要信息', 'warning')
    return
  }
  if (!form.businessLicense) {
    toast('请上传营业执照', 'warning')
    return
  }
  if (!form.idCardFront || !form.idCardBack) {
    toast('请上传身份证正反面', 'warning')
    return
  }
  ...
  try {
    if (company.value) {
      ...
      toast('认证材料已重新提交', 'success')
    } else {
      await request.post('/companies', form)
      toast('认证申请已提交', 'success')
    }
    const res: any = await request.get('/companies/my')
    company.value = res.data
    editing.value = false
