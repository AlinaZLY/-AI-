  ...
  <div class="min-h-screen bg-gray-50 flex items-center justify-center p-4 py-8">
    <div class="w-full transition-all duration-300" :class="form.role === 'enterprise' ? 'max-w-3xl' : 'max-w-sm'">
      <div class="text-center mb-8">
        ...
        <p class="text-gray-400 mt-1 text-sm">学生 / 应届生 / 企业用户注册</p>
      </div>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
        <form @submit.prevent="handleRegister" class="space-y-5">
          ...
          <!-- 身份选择 -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">注册身份 <span class="text-red-500">*</span></label>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">昵称 <span class="text-red-500">*</span></label>
            <input v-model="form.nickname" type="text" required autocomplete="off"
              class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none transition"
              placeholder="显示名称" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">邮箱 <span class="text-xs text-gray-400 font-normal">(选填)</span></label>
          ...
          <div v-if="form.role === 'enterprise'" class="border-t border-gray-100 pt-5 space-y-4">
            <h3 class="text-sm font-semibold text-gray-800">企业认证信息</h3>
            <p class="text-xs text-gray-400 -mt-2">填写后注册即提交审核，也可注册后在"企业认证"页面完善</p>

            <div class="grid sm:grid-cols-2 gap-4">
              <div>
                <label class="block text-xs text-gray-600 mb-1">企业名称</label>
              ...
              <div>
                <label class="block text-xs text-gray-600 mb-1">法人姓名</label>
                <input v-model="enterpriseForm.legalPerson" type="text"
                  class="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none transition" />
              </div>
              <div>
                <label class="block text-xs text-gray-600 mb-1">统一社会信用代码</label>
                <input v-model="enterpriseForm.creditCode" type="text"
                  class="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none transition" />
              </div>
              <div>
                <label class="block text-xs text-gray-600 mb-1">所属行业</label>
            ...
            <!-- 证件上传 -->
            <div>
              <label class="block text-xs text-gray-600 mb-2">证件材料</label>
              <div class="grid grid-cols-3 gap-3">
                <label class="group cursor-pointer">
                  <div class="aspect-[4/3] bg-gray-50 border-2 border-dashed rounded-xl flex flex-col items-center justify-center overflow-hidden transition"
                    ...
                    <img v-if="enterpriseForm.businessLicense" :src="enterpriseForm.businessLicense" class="w-full h-full object-cover" />
                    <template v-else>
                      <svg class="w-8 h-8 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                      <span class="text-[11px] text-gray-400 mt-1">营业执照</span>
                    </template>
                  </div>
                  <input type="file" accept="image/*" class="hidden" @change="(e) => handleUpload(e, 'businessLicense')" />
                </label>
                <label class="group cursor-pointer">
                  <div class="aspect-[4/3] bg-gray-50 border-2 border-dashed rounded-xl flex flex-col items-center justify-center overflow-hidden transition"
                    :class="enterpriseForm.idCardFront ? 'border-blue-300' : 'border-gray-200 group-hover:border-blue-300'">
                    <img v-if="enterpriseForm.idCardFront" :src="enterpriseForm.idCardFront" class="w-full h-full object-cover" />
                    <template v-else>
                      <svg class="w-8 h-8 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                      <span class="text-[11px] text-gray-400 mt-1">身份证正面</span>
                    </template>
                  </div>
                  <input type="file" accept="image/*" class="hidden" @change="(e) => handleUpload(e, 'idCardFront')" />
                </label>
                <label class="group cursor-pointer">
                  <div class="aspect-[4/3] bg-gray-50 border-2 border-dashed rounded-xl flex flex-col items-center justify-center overflow-hidden transition"
                    :class="enterpriseForm.idCardBack ? 'border-blue-300' : 'border-gray-200 group-hover:border-blue-300'">
                    <img v-if="enterpriseForm.idCardBack" :src="enterpriseForm.idCardBack" class="w-full h-full object-cover" />
                    <template v-else>
                      <svg class="w-8 h-8 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                      <span class="text-[11px] text-gray-400 mt-1">身份证反面</span>
                    </template>
                  </div>
                  <input type="file" accept="image/*" class="hidden" @change="(e) => handleUpload(e, 'idCardBack')" />
                </label>
              </div>
            </div>
          </div>
        ...
        <div class="mt-5 pt-5 border-t border-gray-100 text-center">
          <p class="text-sm text-gray-400">
            已有账号？<router-link to="/login" class="text-blue-600 hover:underline font-medium">登录</router-link>
          </p>
        </div>
      </div>
    </div>
...
function handleUpload(e: Event, field: 'businessLicense' | 'idCardFront' | 'idCardBack') {
  const file = (e.target as HTMLInputElement).files?.[0]
  if (!file) return
  const reader = new FileReader()
  reader.onload = () => { enterpriseForm[field] = reader.result as string }
...
async function handleRegister() {
  ...
  try {
    ...
    if (form.role === 'enterprise' && enterpriseForm.name.trim()) {
      try {
        await userStore.login({ username: form.username, password: form.password, captcha: '', captchaKey: '' })
        ...
        toast('注册成功！企业认证已提交审核', 'success')
      } catch {
        toast('注册成功！请登录后在企业认证页面完善资料', 'success')
      }
      userStore.logout()
    } else if (form.role === 'enterprise') {
      toast('注册成功！请登录后完成企业认证', 'success')
    } else {
