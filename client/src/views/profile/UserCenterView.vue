    ...
    <div class="w-56 shrink-0">
      <div class="bg-white rounded-xl border border-gray-100 overflow-hidden sticky top-24">
        ...
        <div class="border-t border-gray-50 p-3">
          <button @click="userStore.logout()" class="w-full text-left px-3 py-2 text-sm text-gray-400 hover:text-red-500 transition-colors">退出登录</button>
        </div>
      </div>
    </div>
    ...
    <div class="flex-1 min-w-0">
      ...
      <div v-if="activeTab === 'profile'" class="bg-white rounded-xl border border-gray-100 p-6">
        <div class="flex justify-between items-center mb-5">
          ...
          <button v-if="!profileEditing" @click="startEditProfile" class="px-4 py-1.5 text-sm border border-gray-200 rounded-lg hover:bg-gray-50 text-gray-600">编辑</button>
          <div v-else class="flex gap-2">
            <button @click="profileEditing = false" class="px-4 py-1.5 text-sm border border-gray-200 rounded-lg hover:bg-gray-50 text-gray-500">取消</button>
            <button @click="saveProfile" :disabled="profileSaving" class="px-4 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">{{ profileSaving ? '保存中...' : '保存' }}</button>
          </div>
        </div>
        <!-- Avatar -->
        <div class="flex items-center gap-4 mb-5">
          ...
          <div v-if="profileEditing" class="text-xs text-gray-400">支持 jpg/png/gif/webp，最大 10MB</div>
        </div>
        <!-- Fields -->
        <div class="grid grid-cols-2 gap-4">
          ...
          <div>
            <label class="block text-xs text-gray-400 mb-1">学校</label>
          ...
          <div>
            <label class="block text-xs text-gray-400 mb-1">专业</label>
          ...
          <div>
            <label class="block text-xs text-gray-400 mb-1">毕业年份</label>
          ...
          <div>
            <label class="block text-xs text-gray-400 mb-1">学历</label>
            <select v-if="profileEditing" v-model="profileForm.degree" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none">
              <option value="">请选择</option><option value="专科">专科</option><option value="本科">本科</option><option value="硕士">硕士</option><option value="博士">博士</option>
            </select>
            <p v-else class="text-sm text-gray-800">{{ userStore.userInfo?.degree || '—' }}</p>
          </div>
        </div>
        <div class="mt-4">
          <label class="block text-xs text-gray-400 mb-1">求职意向</label>
          <input v-if="profileEditing" v-model="profileForm.jobIntention" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="如 前端开发、产品经理" />
      ...
      <div v-if="activeTab === 'resumes'" class="bg-white rounded-xl border border-gray-100 p-6">
        <div class="flex justify-between items-center mb-4">
          <h2 class="text-lg font-semibold text-gray-900">我的简历</h2>
          <div class="flex gap-2">
            ...
            <button @click="showTemplateModal = true" class="px-4 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">从模板创建</button>
          </div>
        </div>
        <div v-if="resumes.length === 0" class="text-center py-8 text-gray-400 text-sm">
          暂无简历，点击"从模板创建"开始
        </div>
      ...
      <div v-if="showTemplateModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="showTemplateModal = false">
        <div class="bg-white rounded-2xl w-full max-w-3xl max-h-[85vh] overflow-y-auto mx-4 shadow-xl">
          <div class="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 flex justify-between items-center z-10">
            <h2 class="text-lg font-bold text-gray-900">{{ selectedTemplate ? '模板预览' : '新建简历' }}</h2>
          ...
          <div v-if="selectedTemplate" class="p-6">
            ...
            <div class="flex justify-between items-center">
              <div>
                ...
                <span class="text-xs text-gray-400">{{ selectedTemplate.category || '通用' }}</span>
              </div>
              <div class="flex gap-2">
                <button @click="selectedTemplate = null" class="px-4 py-2 text-sm border border-gray-200 rounded-lg hover:bg-gray-50 text-gray-600">返回</button>
          ...
          <div v-else class="p-6">
            ...
            <div class="mb-6 p-4 border-2 border-dashed border-gray-200 rounded-xl hover:border-blue-300 transition-colors">
              <label class="flex items-center gap-4 cursor-pointer">
                ...
                <div>
                  <div class="text-sm font-medium text-gray-800">上传简历文件</div>
                  <div class="text-xs text-gray-400">支持 PDF / DOC / DOCX 格式</div>
                </div>
                <input type="file" accept=".pdf,.doc,.docx" class="hidden" @change="handleUploadResume" />
              </label>
            </div>

            <div class="text-sm font-medium text-gray-700 mb-3">或选择模板创建</div>
            ...
            <div v-else class="grid grid-cols-2 sm:grid-cols-3 gap-4">
              <div v-for="tpl in templates" :key="tpl.id"
                ...
                <div class="p-3">
                  ...
                  <div class="text-xs text-gray-400 mt-0.5">{{ tpl.category || '通用' }}</div>
                </div>
              </div>
            </div>
            <div v-if="!templatesLoading && templates.length === 0" class="text-center py-8 text-gray-400 text-sm">暂无模板</div>
          </div>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'applications'" class="bg-white rounded-xl border border-gray-100 p-6">
        <div class="flex justify-between items-center mb-4">
          <h2 class="text-lg font-semibold text-gray-900">投递追踪</h2>
          <router-link to="/applications" class="px-4 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">查看全部</router-link>
        </div>
        <div v-if="applications.length === 0" class="text-center py-8 text-gray-400 text-sm">暂无投递记录</div>
        <div v-else class="space-y-3">
      ...
      <div v-if="activeTab === 'interviews'" class="bg-white rounded-xl border border-gray-100 p-6">
        <div class="flex justify-between items-center mb-4">
          <h2 class="text-lg font-semibold text-gray-900">面试练习</h2>
          <router-link to="/interview" class="px-4 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">开始练习</router-link>
        </div>
        <div v-if="interviews.length === 0" class="text-center py-8 text-gray-400 text-sm">暂无面试记录</div>
        <div v-else class="space-y-3">
          <div v-for="i in interviews" :key="i.id" class="flex justify-between items-center p-3 rounded-lg border border-gray-100 hover:border-blue-100 transition-colors">
            <div>
              <div class="text-sm font-medium text-gray-800">{{ i.jobTitle || '模拟面试' }}</div>
              <div class="text-xs text-gray-400 mt-0.5">{{ formatDate(i.createdAt) }}</div>
            </div>
            <span v-if="i.totalScore" class="text-sm font-semibold text-blue-600">{{ i.totalScore }}分</span>
          </div>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'notifications'" class="bg-white rounded-xl border border-gray-100 p-6">
        <div class="flex justify-between items-center mb-4">
          ...
          <router-link to="/notifications" class="px-4 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">查看全部</router-link>
        </div>
        <div v-if="notifications.length === 0" class="text-center py-8 text-gray-400 text-sm">暂无消息</div>
...
const menuItems = computed(() => {
  ...
  items.push(
    { key: 'notifications', label: '消息通知', badge: unread > 0 ? unread : undefined, icon: 'M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0' },
  )
  return items
})
...
async function handleUploadResume(e: Event) {
  ...
  try {
    ...
    if (newId) {
      const fd = new FormData()
      fd.append('file', file)
      await request.post(`/resumes/item/${newId}/upload`, fd, { headers: { 'Content-Type': 'multipart/form-data' } })
      toast('简历已上传', 'success')
      showTemplateModal.value = false
...
async function handleDeleteResume(r: any) {
  const ok = await confirmDialog(`确定删除「${r.title}」？此操作不可恢复。`, '删除简历')
  if (!ok) return
  try {
