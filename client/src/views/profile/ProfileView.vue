<template>
  <div class="page-shell space-y-6">
    <LoginPrompt v-if="!isLoggedIn" :title="$t('登录后管理个人资料')" :description="$t('登录后可以完善您的个人信息、上传头像、修改密码')" />
    <template v-else>
    <h1 class="text-2xl font-bold text-gray-900">{{ $t('个人资料') }}</h1>

    <!-- Profile Info Card -->
    <section class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
        <h2 class="text-lg font-semibold text-gray-800">{{ $t('基本信息') }}</h2>
      </div>
      <div class="p-6">
        <!-- Avatar -->
        ...
        <form class="space-y-4" @submit.prevent="saveProfile">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('昵称') }}</label>
              <input
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('邮箱') }}</label>
              <input
                v-model="form.email"
                type="email"
                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('联系电话') }}</label>
              <input
                v-model="form.phone"
                type="tel"
                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('性别') }}</label>
              <select
                v-model="form.gender"
                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
              >
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('学校') }}</label>
              <input
                v-model="form.school"
                type="text"
                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('专业') }}</label>
              <input
                v-model="form.major"
                type="text"
                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('毕业年份') }}</label>
              <input
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('学历') }}</label>
              <select
                v-model="form.degree"
                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
              >
                <option value="">{{ $t('请选择') }}</option>
                <option value="专科">{{ $t('专科') }}</option>
                <option value="本科">{{ $t('本科') }}</option>
                <option value="硕士">{{ $t('硕士') }}</option>
                <option value="博士">{{ $t('博士') }}</option>
              </select>
            </div>
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('求职意向') }}</label>
            <input
              v-model="form.jobIntention"
              type="text"
              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
              :placeholder="$t('如 前端开发、产品经理')"
            />
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('个人简介') }}</label>
            <textarea
              v-model="form.bio"
              rows="4"
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('技能标签') }}</label>
    ...
    <section class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
        <h2 class="text-lg font-semibold text-gray-800">{{ $t('修改密码') }}</h2>
      </div>
      <div class="p-6">
        <form class="space-y-4 max-w-md" @submit.prevent="changePassword">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('当前密码') }}</label>
            <input
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('新密码') }}</label>
            <input
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('确认新密码') }}</label>
            <input
...
async function changePassword() {
  ...
  if (newPassword.length < 6) {
    toast(t('新密码至少6位'), 'warning')
    return
  }
  if (newPassword !== confirmPassword) {
    toast(t('两次输入的新密码不一致'), 'warning')
    return
  }
  changingPassword.value = true
  try {
    await request.put('/user/password', { oldPassword, newPassword })
    toast(t('密码已修改'), 'success')
