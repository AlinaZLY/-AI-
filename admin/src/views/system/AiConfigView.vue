<template>
  <div>
    <a-page-header title="AI 配置" sub-title="大模型 API、语音服务与用量管理" />
    ...
    <a-tabs v-model:activeKey="activeTab" type="card">
        ...
        <a-card :bordered="false">
          <a-form layout="vertical" style="max-width: 700px">
            ...
            <a-form-item label="模型选择（参考）">
              <a-select v-model:value="selectedModelRef" placeholder="选择模型参考（仅供显示价格参考）" show-search allow-clear :options="modelOptions" :filter-option="filterOption"
                @change="onModelRefChange" />
              <div style="font-size: 12px; color: #999; margin-top: 4px">
            ...
            <a-row :gutter="16">
              <a-col :span="12">
                <a-form-item label="模型端点 ID / 模型名称">
                  ...
                  <div style="font-size: 12px; color: #999; margin-top: 4px">
                    火山方舟填端点ID (ep-xxx)，OpenAI/智谱填模型名
                  </div>
                </a-form-item>
              </a-col>
              <a-col :span="12">
      ...
      <!-- Tab 4: 用量账单 -->
      <a-tab-pane key="billing" tab="用量账单">
        <a-card :bordered="false">
...
function onModelRefChange(val: string) {
  if (config.aiProvider !== 'volcano') {
    config.arkModelId = val
  }
}
...
// OpenAI 官网模型（platform.openai.com/docs/models），与火山豆包不同
const openaiModels = [
